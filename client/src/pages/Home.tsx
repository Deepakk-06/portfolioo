...
import { ArrowDown, Terminal, Cpu, Code2, Database, Download } from "lucide-react";
    ...
    <div className="max-w-6xl mx-auto">
      ...
      >
        <h2 className="text-3xl md:text-4xl font-mono font-bold tracking-tight">
          <span className="text-muted-foreground opacity-50 mr-2">#</span>{title}
        </h2>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-white/20">
        ...
        <div className="relative z-10 max-w-4xl mx-auto w-full">
          ...
          >
            ...
            <p className="max-w-xl text-lg text-muted-foreground/80 leading-relaxed mb-10">
              I build efficient, reliable software for complex hardware systems. 
              Currently exploring the intersection of control theory and modern autonomous systems.
            </p>
      ...
      <Section id="about" title="About">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6 text-lg text-muted-foreground leading-relaxed">
            <p>
              I am an Electrical & Electronics Engineering student at <strong className="text-foreground">New Horizon College of Engineering</strong>, 
              maintaining a CGPA of <strong className="text-foreground">9.24</strong>.
            </p>
            <p>
              My journey began with a curiosity about how things move. That curiosity evolved into a deep passion for 
              <strong className="text-foreground"> Robotics</strong>, <strong className="text-foreground">Control Systems</strong>, and <strong className="text-foreground">Embedded Software</strong>.
            </p>
            <p>
              Whether it's configuring ROS 2 nodes for autonomous navigation or writing bare-metal firmware for custom PCBs, 
              I enjoy the challenge of bridging the gap between abstract code and physical reality.
            </p>
          </div>
            ...
            <div className="aspect-square bg-gradient-to-br from-zinc-800 to-zinc-950 rounded-2xl border border-white/5 p-8 flex items-center justify-center relative overflow-hidden group">
              ...
              <div className="grid grid-cols-2 gap-4 z-10">
                ...
                <div className="p-6 bg-black/40 backdrop-blur-md border border-white/10 rounded-xl flex flex-col items-center gap-2 hover:border-white/20 transition-colors">
                  ...
                  <span className="font-mono text-xs text-muted-foreground">Hardware</span>
                </div>
                <div className="p-6 bg-black/40 backdrop-blur-md border border-white/10 rounded-xl flex flex-col items-center gap-2 hover:border-white/20 transition-colors">
                  <Database className="w-8 h-8 text-white/80" />
                  <span className="font-mono text-xs text-muted-foreground">Systems</span>
                </div>
                <div className="p-6 bg-black/40 backdrop-blur-md border border-white/10 rounded-xl flex flex-col items-center gap-2 hover:border-white/20 transition-colors">
                  <Code2 className="w-8 h-8 text-white/80" />
                  <span className="font-mono text-xs text-muted-foreground">Software</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <Section id="skills" title="Technical Arsenal">
        {skillsLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
             {[1,2,3].map(i => <div key={i} className="h-40 bg-secondary/20 rounded-xl animate-pulse" />)}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      <Section id="projects" title="Featured Projects">
        {projectsLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             {[1,2,3,4].map(i => <div key={i} className="h-64 bg-secondary/20 rounded-xl animate-pulse" />)}
          </div>
        ) : (
          ...
          <div>
            ...
            <div className="space-y-4 font-mono text-sm text-muted-foreground">
              <div className="flex items-center gap-3">
                ...
                <a href="mailto:deeeeps06@gmail.com" className="hover:text-white transition-colors">deeeeps06@gmail.com</a>
              </div>
              <div className="flex items-center gap-3">
                <span className="w-2 h-2 rounded-full bg-purple-500" />
                <a href="tel:+919606137475" className="hover:text-white transition-colors">+91 9606137475</a>
              </div>
              <div className="flex items-center gap-3">
      ...
      <footer className="py-8 border-t border-border/40 text-center text-muted-foreground/60 font-mono text-xs">
        <p>&copy; {new Date().getFullYear()} Deepak K. Built with React & Tailwind.</p>
      </footer>
    </div>
  );
}
